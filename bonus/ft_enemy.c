...
static int	ft_azar_move(t_enemy *enemy, int index, char **map, int time)
{
	...
	if (time >= 200)
	{
		// ft_printf("%d", index);
		num = rand() % 4;
		if (num == 0 && map[enemy[index].y + 1][enemy[index].x] != '1')
			enemy[index].y += 1;
		else if (num == 1 && map[enemy[index].y - 1][enemy[index].x] != '1')
			enemy[index].y -= 1;
		else if (num == 2 && map[enemy[index].y][enemy[index].x + 1] != '1')
			enemy[index].x +=1;
		else if (num == 3 && map[enemy[index].y][enemy[index].x - 1] != '1')
			enemy[index].x -=1;
		else
...
void	ft_render_enemy(t_vars *vars)
{
	...
	{
		time = ft_azar_move(enemy, i, vars->map_game, time);
